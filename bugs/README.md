...
| [idle-keep-time-not-added.md](idle-keep-time-not-added.md) | Desktop idle "Keep" action | P2 | 🟡 NEEDS PRODUCT DECISION — keep is a by-design no-op (single-timestamp model retains idle time); reported "not added" is most likely the auto-stop/stale-action discard trap or a stale build — verify deployed SHA + telemetry |
