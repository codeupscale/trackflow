...
## Still open (separate, Windows-specific)

Tracked in [desktop-windows-project-dropdown-selection-lost.md](desktop-windows-project-dropdown-selection-lost.md):

- Project dropdown flashes then disappears / selection won't stick — fix candidate: skip unchanged-list rebuild in `loadProjects()` (same class as idle reassign fix).
- "Select Project" control renders empty (no text) on Windows — likely separate Chromium/DPI rendering issue.
- Whether any event still auto-clicks Start (the `startBtn` guards `!projectId`; phantom "tracking" above explains most auto-start reports).
