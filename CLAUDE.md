...
- **Stop + upload on sign-out / quit**: `performLogout()` stops the timer, then — while `apiClient` is still alive — runs a bounded `reconcileTimerState()` + `offlineQueue.flush()` (`LOGOUT_SYNC_BUDGET_MS`, 6s) before teardown; `before-quit` does the same (2.5s) after its local + best-effort server stop, inside a 6s force-exit. `timer_sessions` rows carry `user_id` (`_sessionUserId`, stamped after `getMe()`); every read is scoped via `_ownRowsClause()`, and logout KEEPS the signed-in user's closed-but-unsynced rows instead of the old `DELETE FROM timer_sessions` wipe (which destroyed time tracked offline). Kept rows are closed, so they can never restore as a phantom live timer, and are invisible to another account. A true force-kill runs no JS — recovery there is unchanged: local-first SQLite + reconcile on next launch + server `timer:cleanup-stale`. See `bugs/desktop-signout-quit-unsynced-time-lost.md`.
...
- **Offline screenshot backfill**: `POST /screenshots/presign` accepts a capture for a CLOSED entry when `captured_at` falls inside `[started_at − 2min, ended_at + 2min]` and the entry ended within 7 days (`OFFLINE_BACKFILL_DAYS`); live uploads keep a 5-minute grace. Without this the queue's post-reconnect flush got a 422 — a permanent client error — and every screenshot from an offline session was dropped. See `bugs/offline-screenshots-rejected-after-entry-closed.md`.
- **Idle alert owns the timer while it is up**: an idle decision pending (`isIdlePauseAuthoritative()` = `_isHandlingIdleAction || isIdleAlertActive()`) makes LOCAL state authoritative — `syncOpenTimerFromServerStatus()` returns early and `applyRunningStatusFromServer()` keeps `isTimerPaused` true, so a stale server "running" (the idle pause POST fails when idle starts offline) can never resume tracking behind the alert. `retryIdlePauseIfUnsynced()` re-pushes the pause back-dated to `idleStartedAt` from the `online` handler, the sync tick and the watchdog tick. The idle WATCHDOG (`_idleWatchdogTick`, cap `idle_timeout + 12min`) stands down while an alert is showing — it was closing the popup ~10 min after it appeared, defeating the never-auto-dismiss decision. The popup and tray are LOCKED for the life of the alert: `idle-lock` IPC + `idleLocked` on `get-timer-state` disable Start/Stop/project select, the `start-timer`/`stop-timer` IPC handlers reject, and the tray shows a disabled "Waiting for idle response…". See `bugs/desktop-idle-alert-timer-resumes-on-reconnect.md` and `bugs/desktop-idle-alert-closed-by-idle-watchdog.md`.
- **Today totals**: `todayTotalGlobal` is the COMPLETED all-projects sum (the tick adds live elapsed). It must only ever be fed the never-scoped `all_projects_today_total` — `today_total` is project-scoped whenever a project id is passed, and assigning it made "Today, all projects" collapse to the started/selected project. `start`/`stop`/`switch` responses now include `all_projects_today_total` alongside `status`. See `bugs/desktop-all-projects-total-resets-on-start.md`.
...
| Services | `backend/app/Services/` (Timer, Report, Billing, Audit, Permission, OrganizationStructure, Leave, Payroll, ManualTimeEntry, ProjectTimeReport) |
...
| Desktop services | `desktop/src/main/` (screenshot, activity, idle, offline, keychain, network-monitor, api-client, timer-manager, window-geometry) |
