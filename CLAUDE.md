...
- Filters: `project_id` (single uuid or array), `user_id`, `period=week|month|custom` (`week_of` / `month` / `start_date`+`end_date`), resolved in the org timezone via `TimezoneAwareDateRange`.
- **Role scope** (via `reports.view`): organization=all, project=team, own=self — the same scope contract as the rest of `ReportService`.
...
- Web: `web/src/app/(dashboard)/reports/project-time/page.tsx` + `web/src/components/reports/ProjectTimeFilterBar.tsx` + `web/src/hooks/reports/use-project-time-report.ts`. Export reuses `web/src/lib/download.ts`. Nav: "Project Time" (Analytics group, `reports.view`).
