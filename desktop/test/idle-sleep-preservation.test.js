...
// that replicate index.js's onSuspendCleanup / onResumeAfterSleep exactly, and assert
// the preserve-across-sleep behavior — while proving the normal non-idle lid-close
// keeps the timer running (no hard auto-stop).
...
function buildHarness(detector) {
    ...
    const isIdleAlertActive = () => {
        ...
        const windowLive =
            state.idleAlertWindow && !state.idleAlertWindow.isDestroyed();
    ...
    const reshowIdleAlertAfterResume = (
        idleSeconds,
        idleStartedAt,
        actionId,
    ) => {
        ...
        onResumeAfterSleep: () => {
            ...
            if (preserved && preserved.isIdle && preserved.idleStartedAt) {
                if (state.isTimerRunning) {
                    ...
                    const newActionId = state.detector.setAlertState(
            ...
            } else if (state.isTimerRunning && !isIdleAlertActive()) {
...
describe("Bug B — regressions protected", () => {
    ...
    test("NON-idle lid-close keeps timer running and leaves detector watching", async () => {
        detector.start();
        const { state } = buildHarness(detector);
    ...
    test("'never'/'always' policies (no alert window) keep timer running on sleep", async () => {
        detector.start();
