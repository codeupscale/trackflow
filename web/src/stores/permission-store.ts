    ...
    persist(
        (set, get) => ({
            ...
            hasPermissionWithScope: (key, requiredScope) => {
                ...
                if (granted === "none" && requiredScope !== "none")
                    return false;
                return (
                    (SCOPE_HIERARCHY[granted] ?? 0) >=
            ...
            canAccessModule: (module) => {
                return Object.keys(get().permissions).some((key) =>
        ...
        }),
        {
            ...
            version: 2,
            // Bump version when permission keys change so stale localStorage maps refresh.
            migrate: (persisted) => ({ permissions: {} }),
