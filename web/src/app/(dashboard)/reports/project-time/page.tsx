...
const PERIOD_LABEL: Record<ProjectTimeFilters['period'], (f: ProjectTimeFilters) => string> = {
...
export default function ProjectTimeReportPage() {
  ...
  const { data, isLoading, isError } = useProjectTimeReport(filters, mounted && canView);
  ...
  const patch = (p: Partial<ProjectTimeFilters>) =>
    setFilters((prev) => ({ ...prev, ...p, page: 'page' in p ? (p.page ?? 1) : 1 }));
    ...
    ? [
        {
          icon: Clock,
          label: 'Total Hours',
          value: (summary.total_seconds / 3600).toLocaleString(undefined, {
        ...
        { icon: ListChecks, label: 'Entries', value: summary.entry_count.toLocaleString() },
        { icon: Users, label: 'Resources', value: summary.resource_count.toLocaleString() },
        { icon: Layers, label: 'Projects', value: summary.project_count.toLocaleString() },
      ]
    : [];
  ...
  return (
    <div className="flex flex-col gap-6">
      <PageHeader
        ...
        description="Filter tracked time by project, resource, and period."
        action={
          <div className="flex items-center gap-2">
            <Button
              ...
              disabled={!canExport || exporting !== null || !rows.length}
            >
              {exporting === 'csv' ? (
      ...
      {isError ? (
        <Card>
          <CardContent className="py-12">
            <div className="text-center">
              ...
              <p className="font-medium text-muted-foreground">Failed to load report</p>
        ...
        <EmptyState
          ...
          description="Adjust the filters or period to see project time."
        />
      ) : (
        <Card>
                ...
                <TableBody>
                  {rows.map((row) => (
                    <TableRow key={row.id}>
                      ...
                      <TableCell className="text-sm">{row.project_name}</TableCell>
                      <TableCell className="max-w-[220px]">
                        <span className="block truncate text-sm text-foreground" title={row.task_name ?? ''}>
                          {row.task_name || <span className="text-muted-foreground">—</span>}
                        </span>
                      </TableCell>
      ...
      {totalPages > 1 && (
        <div className="flex items-center justify-between">
          <span className="text-sm text-muted-foreground">
            Page {meta?.current_page ?? filters.page} of {totalPages}
          </span>
          <Pagination className="mx-0 w-auto justify-end">
            <PaginationContent>
              <PaginationItem>
                <PaginationPrevious
                  onClick={() => patch({ page: Math.max(1, filters.page - 1) })}
              ...
              <PaginationItem>
                <PaginationNext
                  onClick={() => patch({ page: Math.min(totalPages, filters.page + 1) })}
