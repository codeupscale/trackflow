...
function transformReportResponse(
    type: ReportType,
    ...
    switch (type) {
        ...
        case "apps": {
            ...
            return {
                ...
                rows: apps.map((a) => ({
                    ...
                    active_app: String(a.active_app ?? a.app_name ?? ""),
                    duration_seconds: Number(
                        a.duration_seconds ?? a.estimated_seconds ?? 0,
                    ),
        ...
        case "payroll": {
            ...
            return {
                ...
                summary: {
                    total_hours: payroll.reduce(
                        (s, p) =>
                            s +
                            Number(
                                (p as Record<string, unknown>).total_hours ?? 0,
                            ),
                        0,
                    ),
                    total_amount: payroll.reduce(
                        (s, p) =>
                            s +
                            Number(
                                (p as Record<string, unknown>).earnings ?? 0,
                            ),
                        0,
                    ),
                },
        ...
        case "attendance": {
            const attendance = (raw.attendance || []) as Record<
                string,
                unknown
            >[];
            return {
    ...
    [
        {
            value: "summary",
            label: "Summary",
            description: "Overview of hours and activity",
        },
        {
            value: "team",
            label: "Team",
            description: "Breakdown by team member",
        },
        {
            value: "projects",
            label: "Projects",
            description: "Hours and costs per project",
        },
        {
            value: "apps",
            label: "Apps & URLs",
            description:
                "Per-employee application usage (use User filter to narrow)",
        },
        {
            value: "payroll",
            label: "Payroll",
            description: "Billable hours and payroll data",
        },
        {
            value: "attendance",
            label: "Attendance",
            description: "Work schedule and attendance",
        },
    ];
...
export default function ReportsPage() {
    ...
    const {
        ...
        queryFn: async () => {
            ...
            return {
                ...
                team_activity_levels: (raw.team_activity_levels ?? []).map(
    ...
    const handleExport = async (exportFormat: "pdf" | "csv") => {
        ...
        try {
            ...
            toast.error(
                (await readBlobError(err)) ?? "Failed to export report",
            );
        } finally {
            setIsExporting(null);
        }
    ...
    const formatColumnName = (key: string) => {
        const labels: Record<string, string> = {
            ...
            duration_seconds: "Time Utilized",
    ...
    // Derived data
    const timePerProject = useMemo(
        () => analytics?.time_per_project ?? [],
        [analytics],
    );
    const teamActivityLevels = useMemo(
        () => analytics?.team_activity_levels ?? [],
        [analytics],
    );
                            ...
                            <div className="flex items-center justify-between mb-3">
                                ...
                                {analytics.hours_change_percent !== null ? (
                                    ...
                                    >
                                        ...
                                        {Math.abs(
                                            analytics.hours_change_percent,
                                        ).toFixed(1)}
                                        %
                                    </span>
                                ) : (
                        ...
                        <CardContent className="p-3">
                            <div className="flex items-center justify-between mb-3">
                                ...
                                {analytics.budget_change_percent !== null ? (
                                    ...
                                    >
                                        {analytics.budget_change_percent >=
                                        0 ? (
                                            <ChevronUp className="size-3" />
                                        ) : (
                                            <ChevronDown className="size-3" />
                        ...
                        {analyticsLoading ? (
                            ...
                            >
                                ...
                                >
                                    ...
                                    />
                                    <Bar
                                        dataKey="avg_activity"
                                        radius={[4, 4, 0, 0]}
                                    >
                                        {teamActivityLevels.map(
                                            (entry, index) => (
                                                <Cell
                                                    key={`cell-${index}`}
                                                    fill={
                                                    ...
                                                    <div className="flex items-center gap-2 min-w-[80px]">
                                                        <div className="flex-1 bg-muted rounded-full h-1">
                                                            <div
                                                                ...
                                                                style={{
                                                                    width: `${Math.min(row.activity_percent, 100)}%`,
                                                                }}
                                                            />
                                                        </div>
